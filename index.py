...
def clickBtn(img, timeout=3, threshold = ct['default'], only_compare = False):
...
def GetDiscordPrint(type):
    with mss.mss() as sct:
        if type == 'basic':
            ...
            if clickBtn(images['farm-chest']):
                time.sleep(10)
                monitor = sct.monitors[1]
                sct_img = sct.grab(monitor)
                img = Image.frombytes("RGB", sct_img.size, sct_img.bgra, "raw", "BGRX")
                output = "./pics/farm.png"
                img.save(output)
...
def clickGreenBarButtons():
    ...
    if len(not_working_green_bars) > 0:
        ...
        # sendMsgWebhook('👆 Coloquei %d bombers para trabalhar #FARINHANELES ' % hero_clicks_cnt)
    ...
    for (x, y, w, h) in not_working_green_bars:
        ...
        # if clickBtn(images['acc1'], only_compare = True):
        #     account = '1'
        # elif clickBtn(images['acc2'], only_compare = True):
        #     account = '2'
        # elif clickBtn(images['acc3'], only_compare = True):
        #     account = '3'
        sendMsgWebhook('👆 Coloquei **%s bombers para trabalhar** ' % (hero_clicks_cnt))
...
def login():
    ...
    if clickBtn(images['select-wallet-2'], timeout = 20):
        ...
        if clickBtn(images['treasure-hunt-icon'], timeout=25):
            ...
            login_attempts = 0
        # time.sleep(15)
    if clickBtn(images['error'], timeout=5):
        logger('🔃 Beta ERROR')
        login_attempts = 0
        pyautogui.hotkey('ctrl','f5')
...
def sendMsgWebhook(msg):
    logger('🏢 Send to webhook')
    now = datetime.now()
    current_time = now.strftime("%H:%M:%S")
    day = now.strftime("%d/%m/%Y")
    config = c['time_intervals']
    url = config['webhooklink']

   
    webhook = DiscordWebhook(url=url, content=msg + current_time + ' ' + day)
    # with open(dir_path+ type +".png", "rb") as f:
    #     webhook.add_file(file=f.read(), filename=dir_path+ type +".png")

    response = webhook.execute()
    

def sendToWebhook(type='basic'):
    tipo = type
    dir_path = './pics/'
    logger('🏢 Send to webhook')
    GetDiscordPrint(tipo)
    now = datetime.now()
    current_time = now.strftime("%H:%M:%S")
    day = now.strftime("%d/%m/%Y")
    config = c['time_intervals']
    ...
    TYPE = {
        'basic': ['📷 BOMB CRYPTO ---> 🕐 TREASURE HUNT: '],
        'farm': ['📷 BOMB CRYPTO ---> 🪙🪙 **FARM-TIME**: '],
       
    }


    if TYPE[type]:
        webhook = DiscordWebhook(url=url, content=TYPE[type][0] + current_time + ' ' + day)
        with open(dir_path+ type +".png", "rb") as f:
            webhook.add_file(file=f.read(), filename=dir_path+ type +".png")
            if type == "farm":
                goToGame()
...
def main():
    ...
    while True:
        ...
        for last in windows:
            ...
            if now - last["sendToDiscord"] > addRandomness(config["timeToSendDiscord"] * 60):
                last["sendToDiscord"] = now 
                sendToWebhook('basic')
                if now - last["sendAmountFarm"] > addRandomness(config["sendAmountFarm"] * 60): 
                    last["sendAmountFarm"] = now 
                    sendToWebhook('farm')
            ...
            if now - last["bug"] > addRandomness( config['refresh_bau'] * 60): #beta
                last["bug"] = now
                pyautogui.hotkey('ctrl','f5')
                time.sleep(10)
                login()
